...
export default function Home() {
  ...
  const todayCourses = projects.flatMap(
    (project) =>
      project.courses
        ?.filter((course) => {
  ...
  const todayCourseNames = todayCourses.map((course) => course.name);
  ...
  const weeklyHours = projects.reduce(
    (sum, project) =>
      ...
      (project.courses?.reduce(
        (courseSum, course) =>
          courseSum +
          (course.todos?.reduce((todoSum, todo) => {
            ...
            if (todoDate >= monday && todoDate <= sunday) {
  ...
  const personalHours = todos
    .filter((todo) => {
      ...
      return todoDate >= monday && todoDate <= sunday;
    })
  ...
  const totalHours = weeklyHours + personalHours;
